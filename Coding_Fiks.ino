...
void GerakMotor(bool jemuranKeluar, int duration) {
  ...
  for (dutyCycle = 200; dutyCycle < 230; dutyCycle++) {
    ledcWrite(pwmChannel, dutyCycle);
    delay(20);
  }
...
void loop() {
  // Membaca nilai analog dari sensor hujan dan sensor cahaya
  ...
  // Mengirimkan kondisi cuaca melalui Telegram jika terjadi perubahan
  if (nilaiSensorHujan != lastSensorHujan || nilaiSensorCahaya != lastSensorCahaya) {
    String message;

    if (nilaiSensorHujan >= 3900 && nilaiSensorCahaya < 400) {
      if (!JemuranKeluar) {
        GerakMotor(true, 1); // Motor bergerak selama 5 detik ke arah kanan
        JemuranKeluar = true;
      ...
      if (JemuranKeluar) {
        GerakMotor(false, 1); // Motor bergerak selama 5 detik ke arah kiri
        JemuranKeluar = false;
      ...
      if (JemuranKeluar) {
        GerakMotor(false, 2); // Motor bergerak selama 5 detik ke arah kiri
        JemuranKeluar = false;
